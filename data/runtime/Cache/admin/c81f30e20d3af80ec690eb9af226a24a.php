<?php if (!defined('THINK_PATH')) exit();?><!doctype html><html><head><meta charset="utf-8" /><meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1" /><link href="__TMPL__public/css/style.css" rel="stylesheet"/><title><?php echo L('website_manage');?></title><script>
	var URL = '__URL__';
	var SELF = '__SELF__';
	...
	//语言项目
	var lang = new Object();
	<?php $_result=L('js_lang');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?>lang.<?php echo ($key); ?> = "<?php echo ($val); ?>";<?php endforeach; endif; else: echo "" ;endif; ?></script></head><body><div id="J_ajax_loading" class="ajax_loading"><?php echo L('ajax_loading');?></div><?php if(($sub_menu != '') OR ($big_menu != '')): ?><div class="subnav"><div class="content_menu ib_a blue line_x"><?php if(!empty($big_menu)): if($big_menu['title']): ?><a class="add fb J_showdialog" href="javascript:void(0);" data-uri="<?php echo ($big_menu["iframe"]); ?>" data-title="<?php echo ($big_menu["title"]); ?>" data-id="<?php echo ($big_menu["id"]); ?>" data-width="<?php echo ($big_menu["width"]); ?>" data-height="<?php echo ($big_menu["height"]); ?>"><em><?php echo ($big_menu["title"]); ?></em></a><?php else: if(is_array($big_menu)): $i = 0; $__LIST__ = $big_menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><a class="add fb J_showdialog" href="javascript:void(0);" data-uri="<?php echo ($val["iframe"]); ?>" data-title="<?php echo ($val["title"]); ?>" data-id="<?php echo ($val["id"]); ?>" data-width="<?php echo ($val["width"]); ?>" data-height="<?php echo ($val["height"]); ?>"><em><?php echo ($val["title"]); ?></em></a><?php endforeach; endif; else: echo "" ;endif; endif; endif; if(!empty($sub_menu)): if(is_array($sub_menu)): $key = 0; $__LIST__ = $sub_menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($key % 2 );++$key; if($key != 1): ?><span>|</span><?php endif; ?><a href="<?php echo U($val['module_name'].'/'.$val['action_name'],array('menuid'=>$menuid)); echo ($val["data"]); ?>" class="<?php echo ($val["class"]); ?>"><em><?php echo ($val['name']); ?></em></a><?php endforeach; endif; else: echo "" ;endif; endif; ?></div></div><?php endif; ?><style type="text/css">
#mall_id{
    width: 200px;
}
</style><form id="info_form" action="<?php echo U(MODULE_NAME.'/add');?>" method="post" enctype="multipart/form-data"><div class="pad_10"><div class="col_tab"><ul class="J_tabs tab_but cu_li"><li class="current"><?php echo L('article_basic');?></li><li><?php echo L('article_seo');?></li></ul><div class="J_panes"><div class="content_list pad_10"><table><tr><td style="width: 80%;"><table width="100%" cellspacing="0" class="table_form"><tr><th style="width: 60px;">商品名称：</th><td style="width: 680px;"><input type="text" name="title" id="J_title" rel="title_color" class="input-text iColorPicker" size="72" value="<?php echo ($info["title"]); ?>" style="color:<?php echo ($info["colors"]); ?>"><input type="hidden" value="<?php echo ($info["colors"]); ?>" name="colors" id="title_color"><a href="javascript:;" class="color_picker_btn"><img class="J_color_picker" data-it="J_title" data-ic="J_color" src="__STATIC__/images/color.png"></a></td></tr><tr><th>商品标签：</th><td><input type="text" name="tags" id="J_tags" class="input-text" size="65" value="<?php echo ($tags); ?>"/><input type="button" value="<?php echo L('auto_get');?>" id="J_gettags" name="tags_btn" class="btn"/></td></tr><tr><th>商品链接：</th><td><input type="text" name="url" class="input-text" size="60"value="<?php echo ($info["url"]); ?>"/></td></tr><tr><th>商品价格：</th><td><input type="text" name="prices" class="input-text" size="60" value="<?php echo ($info["prices"]); ?>"/></td></tr><tr><th>所属商城：</th><td><div class="J_mall_index"><select class="index"></select><select class="list" name="mall_id"></select></div></td></tr><tr><th>商品图片：</th><td><?php if($info['zs_images']): ?><div class="img_border"><span class="attachment_icon J_attachment_icon" file-type="image" file-rel="<?php echo attach($info['zs_images'],'zhaoshang');?>"><img src="<?php echo attach($info['zs_images'],'zhaoshang');?>" width="60" height="60" /></span></div><input type="hidden" name="zs_images" value="<?php echo ($info["zs_images"]); ?>" /><?php else: ?>
										上传：<input type="file" name="img" class="input-text"  style="width:200px;" />
                                        &nbsp;&nbsp;<input type="text" class="input-text J_tip" data-default="或者输入外部图片地址" name="img_url" value="<?php echo ($info["img"]); ?>" size="55"/><?php endif; ?></td></tr><tr><th> 开始时间：</th><td><input type="text" name="start_time" id="J_start_time" class="date J_date_picker" size="20" value="<?php echo date('Y-m-d H:i',time());?>"/></td></tr><tr><th> 结束时间：</th><td><input type="text" name="end_time" id="J_end_time" class="date J_date_picker" size="20" value="<?php echo date('Y-m-d H:i',time());?>"/></td></tr><tr><th>附加图片：</th><td><div>
                                        上传：<input type="file" name="moreImg[]" class="input-text"  style="width:200px;" />
                                        &nbsp;&nbsp;<input type="text" class="input-text J_tip" placeholder="或者输入外部图片地址" name="moreImg[]" value="" size="55"/></div><br/><div class="moreImgDiv"><button id="addMoreLine">添加</button></div></td></tr><tr><th>物流信息：</th><td><textarea name="delivery_info" id="delivery_info" style="width:90%;height:100px;resize:none;"><?php echo ($info["delivery_info"]); ?></textarea></td></tr><tr><th>提示：</th><td><textarea name="tips" id="tips" style="width:90%;height:100px;resize:none;"><?php echo ($info["tips"]); ?></textarea></td></tr><tr><th>链接：</th><td><div><label>URL: </label><input type="text" name="moreLink[]" class="input-text" value="" placeholder="链接地址" /><input type="text" name="moreDes[]" class="input-text" placeholder="描述" value="" /><label> 点击数：</label><input type="text" name="moreCount[]" class="input-text" value="" style="width:20px" /></div><br/><div class="moreLinkDiv"><button id="addMoreLink">添加</button></div></td></tr><tr><th>所属品牌：</th><td><div class="J_brand_index"><select class="brandlist" name="brand_id"></select></div></td></tr><tr><th>语言：</th><td><select name="lang"><option value=""></option><option value="cn">中文</option><option value="fr">法语</option></select></td></tr><tr><td colspan="2"><textarea name="info" id="info" style="width:100%;height:350px;visibility:hidden;resize:none;"><?php echo ($info["info"]); ?></textarea></td></tr></table></td><td valign="top"><table width="100%" cellspacing="0" class="table_form inner_table"><tr><td><div class="box"><div class="box_title">所属栏目：</div><div class="box_content" style="height: 250px;overflow-y: scroll;"><?php echo ($cate_tree); ?></div></div></td></tr><tr><td>作者：<input type="text" name="uname" class="input-text" size="28" value="<?php echo ($_SESSION['admin']['username']); ?>"/></td></tr><tr><td>
                                    热门：
                                    <label><input type="radio" name="is_hot" class="radio_style" value="1" <?php if($info['is_hot'] == 1): ?>checked="checked"<?php endif; ?>/> 是</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
          							<label><input type="radio" name="is_hot" class="radio_style" value="0"<?php if($info['is_hot'] == 0): ?>checked="checked"<?php endif; ?>/>否</label></td></tr><tr><td>
                                    推荐：
...
</script><?php if(isset($list_table)): ?><script type="text/javascript" src="__STATIC__/js/jquery/plugins/listTable.js"></script><script>
$(function(){
	$('.J_tablelist').listTable();
});
</script><?php endif; ?><script src="__STATIC__/js/jquery/plugins/colorpicker.js"></script><script src="__STATIC__/js/kindeditor/kindeditor-min.js"></script><script>
