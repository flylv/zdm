...
class postAction extends backendAction {
    var $list_relation = true;    public function _initialize() {
        parent::_initialize();
        $this->_mod = D('post');
        ...
        $map['collect_flag'] = $collect_flag;        ($time_start = $this->_request('time_start', 'trim')) && $map['post_time'][] = array('egt', strtotime($time_start));
        ...
        $selected_ids = '';
        if ($cate_id) {
            $id_arr = $this->_cate_mod->get_child_ids($cate_id, true);            $res = D("post_cate_re")->where("cate_id in(" . implode(',', $id_arr) . ")")->select();
            $ids = "0";
            foreach ($res as $val) {
                $ids .= "," . $val['post_id'];
            }
	...
	if (!empty($_POST['zs_images'])) {
        ...
        $cate_tree = $this->_get_cate_tree(get_cate_tree(D("post_cate")), $ids);        $this->assign('cate_tree', $cate_tree);        $this->assign("mall_title", D("mall")->where(array('id' => $data['mall_id']))->getField("title"));
        $tag_list = D("post_tag")->relation(true)->where($where)->select();        foreach ($tag_list as $key => $val) {
            $tags .= " " . $val['tag']['name'] . " ";
        }
    ...
    function collect() {
        ...
        if ($act == 'search') {
            ...
            $pager = new Page($data['total'], $data['page_size']);            $this->assign('page', $pager->show());
            $this->assign('p', $p);
            ...
            foreach ($resdata['items'] as $key => $val) {
                if (in_array($val['id'], $ids)) {
                    ...
                    $data['post_key']=$this->get_post_key($data['title']);
                    $data=D('post')->parse_data($data);     
                    $data['mall_id']=$this->_request('mall_id', 'intval',0);                    D('post')->add($data);
